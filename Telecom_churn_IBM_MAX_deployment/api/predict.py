...
class ModelPredictAPI(PredictAPI):
    ...
    def post(self):
        ...
        # input_data = args['file'].read() UPDATED
        input_data = pd.read_csv(args['file']) #ADDED
        print('********')#ADDED
        print(input_data)#ADDED

        preds = self.model_wrapper.predict(input_data)

        # Modify this code if the schema is changed
        #label_preds = [{'label_id': p[0], 'label': p[1], 'probability': p[2]} for p in [x for x in preds]] COMMENTED
